...
public class ColliderForRifleMagazine2 : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.tag == "RifleMagazine2" && RifleParams.isEmptyMagazine == true)
        {
            if (other.GetComponent<MagazinRifle>().mode == 2)
            {
                ...
                other.GetComponent<MagazinRifle>().mode = 1;
                RifleParams.isEmptyMagazine = false;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.P)) Debug.Log("IsEmptyMag = " + RifleParams.isEmptyMagazine);
