...
public class AssaultRifle : MonoBehaviour
{
    ...
    ColliderForAssaultRifleMagazine colliderForRifle;
    float nextShoot;
    ...
    void Start()
    {
        ...
        colliderForRifle = ReloadCollider.GetComponent<ColliderForAssaultRifleMagazine>();
    }
    ...
    void Update()
    {
        if (scriptHand.currentAttachedObject != null)
        {
            if (scriptHand.currentAttachedObject.GetComponent<AssaultRifle>())
            {
                ...
                else if (magazine.GetComponent<AssaultRifleMagazine>().ammo <= 0 || !hasSlide)
                {
                    gunAnimator.enabled = false;
                }
                ...
                {
                    if (Time.time > nextShoot && magazine.GetComponent<AssaultRifleMagazine>().ammo > 0 && AssaultRifleParams.isEmptyMagazine == false && hasSlide)
                    {
                ...
                if(Input.GetKeyDown(KeyCode.U))
                {
                    ...
                    Debug.Log("AssaultRifleMagazine.ammo = " + magazine.GetComponent<AssaultRifleMagazine>().ammo);
                    Debug.Log("AssaultRifleParams.isEmptyMagazine = " + AssaultRifleParams.isEmptyMagazine);
    ...
    void Shoot()
    {
        ...
        //tempBullet.GetComponent<Rigidbody>().isKinematic = true;
            ...
            magazine.GetComponent<AssaultRifleMagazine>().ammo--;
    ...
    void Reload()
    {
        ...
        {
            if (AssaultRifleParams.isEmptyMagazine == false)
            {
    ...
    void ToggleMagMode()
    {
        ...
        {
            if (magazine.GetComponent<AssaultRifleMagazine>().mode == 1)
            {
                if (DistanceFromMagToPlace(magazine, PlaceForMagazine) >= 0.8f)
                {
                    magazine.GetComponent<AssaultRifleMagazine>().mode = 2;
                    magazine = null;
                }
            }
        }
    ...
    void Detach()
    {
        if (magazine)
        {
            ...
            AssaultRifleParams.isEmptyMagazine = true;
            hasSlide = false;
        }
    }
